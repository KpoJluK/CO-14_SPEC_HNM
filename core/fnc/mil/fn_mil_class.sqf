...

{
    ...
    private _type_motorized_armed_classes = _allclass_f select {((_x isKindOf "Air") || (_x isKindOf "Helicopter") || (_x isKindOf "Tank") || (_x isKindOf "Car"))};
    if (isNil "_type_motorized_armed_classes") then {
        _type_motorized_armed_classes = [];
    };
    _type_motorized_armed append (([_type_motorized_armed_classes] call BTC_fnc_find_veh_with_turret) select 0);
